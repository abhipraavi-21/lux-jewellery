...
import { createId, type CategoryRecord } from "@/lib/site-data";
...
function AdminCategoriesPage() {
  ...
  const [draft, setDraft] = useState<CategoryRecord | null>(null);
  ...
  const rows = useMemo(
    () =>
      categories.filter((category) =>
        query.trim() === ""
          ? true
          : category.name.toLowerCase().includes(query.toLowerCase()) || category.description.toLowerCase().includes(query.toLowerCase()),
      ),
    [categories, query],
  );
  ...
  return (
    <div className="space-y-6">
      <section className={adminSectionClass.replace("p-6", "p-5")}>
        <div className="flex flex-col gap-4 md:flex-row md:items-end md:justify-between">
          <label className="flex w-full max-w-md items-center gap-3 rounded-2xl border border-slate-200 px-4 py-3">
            ...
            <input className="w-full bg-transparent text-sm outline-none placeholder:text-slate-900/35" placeholder="Search categories..." value={query} onChange={(event) => setQuery(event.target.value)} />
          </label>
          <button
            type="button"
                ...
                return (
                  <tr key={category.id} className="border-t border-slate-200">
                    <td className="px-5 py-4">
                      <img src={category.image} alt={category.name} className="size-14 rounded-2xl object-cover ring-1 ring-slate-200" />
                    </td>
                    ...
                    <td className="px-5 py-4">
                      <div className="flex gap-2">
                        <ActionButton label="Edit" icon={Pencil} onClick={() => setDraft(structuredClone(category))} />
                        <ActionButton
                          label="Delete"
                          icon={Trash2}
                          destructive
      ...
      {draft && (
        <AdminModal title={draft.id ? "Edit Category" : "Add Category"} subtitle="Maintain product category taxonomy." onClose={() => setDraft(null)}>
          <div className="grid gap-4 md:grid-cols-2">
            <Field label="Category Name" required>
              <input className={inputClass} value={draft.name} onChange={(event) => setDraft({ ...draft, name: event.target.value })} />
            </Field>
            <Field label="Category Image" required>
              <div className="space-y-3">
                ...
                {draft.image ? (
                  <div className="overflow-hidden rounded-2xl border border-slate-200 bg-slate-50">
                    <img src={draft.image} alt="Category preview" className="h-32 w-full object-cover" />
                  </div>
                ) : null}
              </div>
            </Field>
